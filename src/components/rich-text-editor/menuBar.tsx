...
export function Menubar({ editor }: iAppProps) {
  ...
  useEffect(() => {
    const updateMarks = () => {
      setActiveMarks({
        ...
        // note: heading here indicates *any* heading is active; you can check level if needed
        heading: editor.isActive("heading"),
        bulletList: editor.isActive("bulletList"),
        orderedList: editor.isActive("orderedList"),
        // textAlign check uses lowercase 'textAlign'
  ...
  const toolbarItems = [
    ...
    // unique keys for headings
    { name: "heading1", icon: Heading1, tooltip: "Heading1", command: () => editor.chain().focus().toggleHeading({ level: 1 }).run() },
    { name: "heading2", icon: Heading2, tooltip: "Heading2", command: () => editor.chain().focus().toggleHeading({ level: 2 }).run() },
    { name: "heading3", icon: Heading3, tooltip: "Heading3", command: () => editor.chain().focus().toggleHeading({ level: 3 }).run() },
  ...
  const historyItems = [
    { name: "undo", icon: Undo, tooltip: "Undo", command: () => editor.chain().focus().undo().run(), can: () => editor.can().undo() },
    { name: "redo", icon: Redo, tooltip: "Redo", command: () => editor.chain().focus().redo().run(), can: () => editor.can().redo() },
  ];

  return (
    <div className="border-b border-input rounded-t-lg p-2 bg-card flex flex-wrap gap-1 items-center">
      <TooltipProvider>
        {/* toolbarItems: ensure key on outermost element */}
            ...
            <Tooltip key={name}>
              <TooltipTrigger asChild>
                <Toggle
                  ...
                  className={cn("transition-colors", activeMarks[name] && "bg-muted text-muted-foreground")}
                >
            ...
            <Tooltip key={name}>
              <TooltipTrigger asChild>
                <Toggle
                  ...
                  onPressedChange={command}
                  className={cn("transition-colors", alignment === name && "bg-muted text-muted-foreground")}
                >
                  <Icon className="h-4 w-4" />
                </Toggle>
              </TooltipTrigger>
        ...
        <div className="flex gap-2 items-center border-l pl-4 ml-2">
          {historyItems.map(({ name, icon: Icon, tooltip, command, can }) => (
            <Tooltip key={name}>
              <TooltipTrigger asChild>
                <Button size="sm" variant="ghost" type="button" onClick={command} disabled={!can()} className="transition-colors">
                  <Icon className="h-4 w-4" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{tooltip}</TooltipContent>
